...
def invoke_agent(repo, cfg, feedback=""):
    ...
    if not template:
        raise RuntimeError("ANTIGRAVITY_COMMAND is not configured. Set it to the Antigravity CLI command/template containing {repo} and {task}.")
    task = (
        f"Work autonomously in {repo}. Inspect the entire repository, documentation, issues/TODOs and existing tests. "
        "Complete the highest-priority unfinished engineering work. Implement it, generate or update tests, build and test it, "
        "fix failures, and verify the result. For web/UI work, use browser verification when available. "
        "If a required skill, library, image, icon, font or other asset is missing, research an appropriate legitimate source online, "
        "prefer clearly licensed/free assets, add it to the project, and verify that it loads. "
        "Never download suspicious executables or expose secrets. Do not declare completion until the work is actually implemented. "
        f"{feedback}"
    ).strip()
    command = template.replace("{repo}", str(repo)).replace("{task}", task)
    log(f"Launching Antigravity for {repo}", cfg)
    return subprocess.call(shlex.split(command, posix=False), cwd=repo)
...
def process_repo(repo, cfg, state):
    ...
    try:
        ...
        for attempt in range(cfg.max_fix_attempts + 1):
            code = invoke_agent(repo, cfg, feedback)
            ...
            feedback = "Previous attempt failed verification. Fix these failures before continuing:\n" + "\n".join(
                f"{x['name']}: {x['output'][-3000:]}" for x in verification["checks"] if not x["ok"]
            )
